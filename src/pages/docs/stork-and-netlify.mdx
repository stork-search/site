...
<Wide>
  <Codeblock filename="build.sh" lang="bash">
    {`#!/usr/bin/env bash
wget https://files.stork-search.net/releases/v1.2.0/stork-ubuntu-latest
chmod +x stork-ubuntu-latest
ELEVENTY_ENV=production npx @11ty/eleventy --config=eleventy.js
./stork-ubuntu-latest build --input _site/stork-posts.toml --output _site/stork-posts.st`}
  </Codeblock>
</Wide>
